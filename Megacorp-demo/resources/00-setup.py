...
bucket_name = "dbfs:/FileStore/shared_uploads/workshop_demo/formatted_workshop_data" # enter path of formatted workshop data
mount_name = "demo-resources"
...
#   spark.sql("""drop database if exists {} CASCADE""".format(dbName))
  ...
  spark.read.format("json") \
            ...
            .load(f"{bucket_name}/turbine/power/raw") \
  ...
  COPY INTO turbine_status_gold
    FROM '{bucket_name}/turbine/status'
    FILEFORMAT = PARQUET;
  """)
...
if reset_all:
  ...
  LOCATION '{bucket_name}/turbine/gold-data-for-ml'
  """)
